...
class Stats extends React.Component {
  ...
  handleMqtt( topic, payload ) {
      if (this.state.total != payload.total){
          this.setState({totalCount: payload.total})};
    switch ( topic ) {
  ...
  calculateDuration( input ) {
    ...
    console.log('out is ', input.duration)
    const format = moment.utc( input.duration*100, "ss" ).format( "mm:ss" );
